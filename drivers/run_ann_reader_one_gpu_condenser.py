...
import os
import torch
from data.msmarco_data import GetTrainingDataProcessingFn, GetTripletTrainingDataProcessingFn, GetTripletTrainingDataProcessingFn_add_origin
...
from model.models_reader_condenser import MSMarcoConfigDict, ALL_MODELS
...
def train(args, model, tokenizer, query_cache, passage_cache):
    ...
    task_dir = '../log/task_loss/'+args.model_type
    div_dir = '../log/div_loss/'+args.model_type
    equal_dir = '../log/equal_loss/'+args.model_type
    folder = os.path.exists(task_dir)
    if not folder:
        os.makedirs(task_dir)

    folder = os.path.exists(div_dir)
    if not folder:
        os.makedirs(div_dir)

    folder = os.path.exists(equal_dir)
    if not folder:
        os.makedirs(equal_dir)

    writer_task_loss =SummaryWriter(log_dir=task_dir)
    writer_div_loss = SummaryWriter(log_dir=div_dir)
    writer_equal_loss = SummaryWriter(log_dir=equal_dir)

    args.train_batch_size = args.per_gpu_train_batch_size * max(1, args.n_gpu)
    real_batch_size = args.train_batch_size * args.gradient_accumulation_steps * 1
    ...
    #         print(i)
    #     for i in torch.load(
    #         os.path.join(args.model_name_or_path, "optimizer.pt"))['param_groups']:
    #         print(i)
    #     # print(torch.load(
    #     #     os.path.join(args.model_name_or_path, "optimizer.pt"))['param_groups'])
    ...
    while global_step < args.max_steps:
        ...
        if step % args.gradient_accumulation_steps == 0 and global_step % args.logging_steps == 0:
            ...
            if ann_path is not None and ann_no != last_ann_no:
                print("Training on new add data at %s", ann_path)
                ...
                aligned_size = (len(ann_training_data) // 1) * 1
                ann_training_data = ann_training_data[:aligned_size]

                print("Total ann queries: %d", len(ann_training_data))
                if args.triplet:
                    train_dataset = StreamingDataset(
                        ann_training_data, GetTripletTrainingDataProcessingFn_add_origin(
                            args, query_cache, passage_cache,tokenizer.pad_token_id))
                else:
                    train_dataset = StreamingDataset(
        ...
        if args.triplet:
            inputs = {
                ...
                "query_len":batch[6].long(),
                "input_ids_reader": batch[7].long(),
                "attention_mask_reader": batch[8].long(),
                "input_ids_b": batch[9].long(),
                "attention_mask_b": batch[10].long()}
        else:
            inputs = {
        ...
        loss_task = outputs[1]
        loss_div = outputs[2]
        loss_equal = outputs[3]
        writer_task_loss.add_scalar('loss_task', loss_task, global_step=count_step_all)
        writer_div_loss.add_scalar('loss_div', loss_div, global_step=count_step_all)
        writer_equal_loss.add_scalar('loss_equal', loss_equal, global_step=count_step_all)
        count_step_all += 1
        if args.n_gpu > 1:
