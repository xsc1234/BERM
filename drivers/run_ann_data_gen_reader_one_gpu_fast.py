...
from utils.eval_mrr import embedding_inference
import csv
import copy
import transformers
...
def StreamInferenceDoc(args, model, fn, prefix, f, is_query_inference=True):
    ...
    # preserve to memory
    full_embedding = _embedding
    full_embedding2id = _embedding2id
...
def generate_new_ann(
    ...
    # with passage_cache as emb: #GetProcessingFn读取embeddingcache里面的每一条数据，提取出input_id来，然后加上attention之类的，做为model的输入
    #     # 然后进入模型获得embedding
    ...
    passage_embedding, passage_embedding2id = embedding_inference(
        args, passage_path, model, fn, args.per_gpu_eval_batch_size, 2, False)
    print("***** Done passage inference *****")
...
def get_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="passage_path of collections",
    )
...
def ann_data_gen(args):
    ...
    while args.end_output_num == -1 or output_num <= args.end_output_num:
        ...
        if next_checkpoint == last_checkpoint:
            ...
            print("start generate ann data number %d", output_num)
            print("next checkpoint at " + next_checkpoint)
            generate_new_ann(
                args,
                output_num,
                next_checkpoint,
            ...
            if args.inference:
                break
            print("finished generating ann data number %d", output_num)
            output_num += 1
