...
from data.process_fn import triple_process_fn, triple2dual_process_fn,four_process_fn
...
import joblib
marco_path = ''
score_list = joblib.load('{}/score_tok_128_lower'.format(marco_path))
...
class NLL_reader_one_hot_gelu_equal_cls(EmbeddingMixin_reader):
    def forward(
            self,
            train_step,
            sample_ids,
        ...
        for i in range(juhao_matrix.shape[0]):
            ...
            if bz_idx == last_bz:
                ...
                target = torch.Tensor(score_list[sample_ids[last_bz]]).unsqueeze(0).to(logist_dis.device)
                count += 1
...
class RobertaDot_NLL_LN_reader_one_hot_gelu_equal_cls(NLL_reader_one_hot_gelu_equal_cls, RobertaForSequenceClassification):
    """None
    Compress embedding to 200d, then computes NLL loss.
    """

    def __init__(self, config, model_argobj=None):
        NLL_reader_one_hot_gelu_equal_cls.__init__(self, model_argobj)
        RobertaForSequenceClassification.__init__(self, config)
        self.embeddingHead = nn.Linear(config.hidden_size, 768)
        self.norm = nn.LayerNorm(768)
        self.apply(self._init_weights)
...
default_process_fn = four_process_fn
...
configs = [
    ...
    MSMarcoConfig(name="reader_one_hot_gelu_equal_cls",
                  model=RobertaDot_NLL_LN_reader_one_hot_gelu_equal_cls,
                  use_mean=False,
                  ),
]
