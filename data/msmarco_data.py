...
#sys.path.append('/data/home/xushicheng/ANCE-raw/')
import os
import torch
sys.path += ['../']
import gzip
...
def GetProcessingFn(args, query=False):
    def fn(vals, i):
        ...
        all_input_ids_a = torch.tensor(
            [f[1] for f in passage_collection], dtype=torch.int)  #[1,len_id]
...
def GetProcessingFn_add_origin(args, query=False):
    def fn(vals, i):
        passage_len, passage = vals
        max_len = args.max_query_length if query else args.max_seq_length
        ...
        token_type_ids = ([0] if query else [1]) * passage_len + [0] * pad_len
        attention_mask = [1] * passage_len + [0] * pad_len
        origin_passage = passage[:passage_len]
        passage_collection = [(i, passage, attention_mask, token_type_ids,origin_passage)]
        if not query:
            passage_id = passage_collection[0][1].copy()
            #中点处添加句号 注意，bert是1012 roberta是4
            # if not 4 in passage_id:
            #     passage_id[passage_len // 2] = 4
            if not 1012 in passage_id:
                passage_id[passage_len // 2] = 1012
            query2id_tensor = torch.tensor(
                [f[0] for f in passage_collection], dtype=torch.long)
            all_input_ids_a = torch.tensor(
                [passage_id], dtype=torch.int)
            all_attention_mask_a = torch.tensor(
                [f[2] for f in passage_collection], dtype=torch.bool)
            all_token_type_ids_a = torch.tensor(
                [f[3] for f in passage_collection], dtype=torch.uint8)
            all_origin_input_ids_a = torch.tensor([passage_collection[0][4]], dtype=torch.long) #[1,sequence_len]
        else:
            query2id_tensor = torch.tensor(
                [f[0] for f in passage_collection], dtype=torch.long)
            all_input_ids_a = torch.tensor(
...
def GetTripletTrainingDataProcessingFn(args, query_cache, passage_cache):
    def fn(line, i):
        ...
        pos_data = GetProcessingFn(
            args, query=False)(
            passage_cache[pos_pid], pos_pid)[0] #单条数据样本的input_id,attention_mask,type_id
...
def GetTripletTrainingDataProcessingFn_add_origin(args, query_cache, passage_cache, pad_token_id):
    def fn(line, i):
        line_arr = line.split('\t')
        qid = int(line_arr[0])
        pos_pid = int(line_arr[1])
        ...
        query_data = GetProcessingFn_add_origin(
            args, query=True)(
            query_cache[qid], qid)[0]
        pos_data = GetProcessingFn_add_origin(
            args, query=False)(
            passage_cache[pos_pid], pos_pid)[0] #单条数据样本的input_id,attention_mask,type_id

        query_data_list = query_data[3].tolist()
        pos_data_list = pos_data[3].tolist()
        max_len = args.max_reader_length
        if len(query_data_list) + len(pos_data_list) > max_len:
            pos_origin = pos_data_list[:max_len-len(query_data_list)]
        else:
            pos_origin = pos_data_list


        pad_len = max(0, max_len - (len(query_data_list) + len(pos_origin)))
        attention_mask_reader = [1] * (len(query_data_list) + len(pos_origin)) + [0] * pad_len
        input_id_reader = query_data_list+pos_origin + [pad_token_id] * pad_len
        ...
        # max_len = args.max_reader_length
        # if query_data[3].shape[1] + pos_data[3].shape[1] > max_len:
        #     pos_origin = pos_data[3][:max_len-query_data[3].shape[1]]
        # else:
        #     pos_origin = pos_data[3]
        #
        #
        # pad_len = max(0, max_len - (len(query_data[3]) + len(pos_origin)))
        # attention_mask_reader = [1] * (len(query_data[3]) + len(pos_origin)) + [0] * pad_len
        # input_id_reader = query_data[3]+pos_origin + [pad_token_id] * pad_len
        # input_id_reader = torch.tensor([input_id_reader], dtype=torch.int)
        # attention_mask_reader = torch.tensor([attention_mask_reader], dtype=torch.bool)
        # print(query_data[0])
        # print(input_id_reader)
        for neg_pid in neg_pids:
            neg_data = GetProcessingFn_add_origin(
                args, query=False)(
                passage_cache[neg_pid], neg_pid)[0]
            yield (query_data[0], query_data[1], query_data[2], pos_data[0], pos_data[1], pos_data[2],len(query_data_list),input_id_reader,attention_mask_reader,
                   neg_data[0], neg_data[1], neg_data[2])
